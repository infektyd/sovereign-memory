...
SOCKET_PATH = os.environ.get("SOVEREIGN_SOCKET_PATH", "/tmp/sovereign.sock")
DB_PATH = os.path.expanduser(os.environ.get("SOVEREIGN_DB_PATH", DEFAULT_CONFIG.db_path))
VAULT_PATH = os.path.expanduser(os.environ.get("SOVEREIGN_VAULT_PATH", DEFAULT_CONFIG.vault_path))
DEFAULT_AGENT_ID = os.environ.get("SOVEREIGN_DEFAULT_AGENT_ID", "default")
...
def get_agent(agent_id: str = DEFAULT_AGENT_ID) -> SovereignAgent:
...
def _enrich_recall_results(markdown: str) -> str:
    ...
    def _enrich_block(match):
        ...
        if os.path.isabs(file_path) and os.path.isfile(file_path):
            ...
            candidate = os.path.join(VAULT_PATH, file_path)
...
def _format_recall_results(results) -> str:
    """Normalize package and legacy recall results to the bridge markdown format."""
    if isinstance(results, str):
        return _enrich_recall_results(results)
    ...
    for idx, item in enumerate(results):
        if not isinstance(item, dict):
            parts.append(f"### result-{idx}.md (score=0.000)\n{item}")
            continue
        source = item.get("filename") or item.get("path") or item.get("source") or f"result-{idx}.md"
        score = item.get("score", 0)
        heading = item.get("heading_context") or item.get("heading") or ""
        text = item.get("chunk_text") or item.get("text") or item.get("content") or ""
        title = f"{source} — {heading}" if heading else str(source)
        try:
            score_text = f"{float(score):.3f}"
        except (TypeError, ValueError):
            score_text = "0.000"
        parts.append(f"### {title} (score={score_text})\n{text}")
    return "\n\n".join(parts)


def _search_vault_for_file(filename: str) -> str:
    """Search the configured vault recursively for a file by name."""
...
def _recall_exact_learnings(query: str, agent_id: str = "", layer: str = None, limit: int = 5) -> str:
    ...
    The bridge smoke test requires write -> recall round-trips, so recall
    merges these exact learning hits ahead of vector/wiki results.
    """
    if not query.strip():
        return ""
    try:
...
def _recall_raw(query: str, agent_id: str = DEFAULT_AGENT_ID, layer: str = None, limit: int = 5) -> str:
    """
    ...
    if layer == "knowledge":
        ...
        enriched = _format_recall_results(results)
    ...
    enriched = _format_recall_results(raw)
    return "\n\n".join(part for part in [exact, enriched] if part)


def _learn(content: str, agent_id: str = DEFAULT_AGENT_ID, category: str = "general",
...
def _read_file(key: str, agent_id: str = "") -> str:
    ...
    candidate = os.path.join(VAULT_PATH, key)
    if os.path.isfile(candidate):
...
class SovereignHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/health":
            ...
            try:
                ...
                if text:
                    self._send_json({"results": [{"text": text, "path": key}]})
                else:
            ...
            agent_id = query.get("agent_id", [DEFAULT_AGENT_ID])[0]
            try:
                a = get_agent(agent_id)
    ...
    def do_POST(self):
        ...
        if path == "/learn":
            ...
            default_id = agent_id if agent_id else DEFAULT_AGENT_ID
