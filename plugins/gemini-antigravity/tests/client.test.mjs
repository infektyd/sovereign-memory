...
import { buildStatusReport, formatRecall, parseSovrdJson } from "../dist/gemini_antigravity.js";
...
test("formatRecall returns concise markdown with query and provenance", () => {
  const formatted = formatRecall("socket health", {
    results: "### daemon.md (score=1.000)\nUse /tmp/gemini_antigravity.sock for local health.",
  ...
  }, [
    {
      ...
      title: "Socket health",
      snippet: "Gemini Anti Gravity should check /tmp/gemini_antigravity.sock before using recall.",
      score: 61,
    },
  ]);

  assert.match(formatted, /Query: socket health/);
  assert.match(formatted, /agent=gemini_antigravity/);
  assert.match(formatted, /AI Context Pack/);
  assert.match(formatted, /\[\[wiki\/sessions\/socket-health\]\]/);
  assert.match(formatted, /Use \/tmp\/gemini_antigravity.sock/);
});
