...
const AFM_URL = process.env.GEMINI_ANTIGRAVITY_AFM_PREPARE_TASK_URL ?? "http://127.0.0.1:11437/v1/chat/completions";
const AFM_HEALTH_URL = process.env.GEMINI_ANTIGRAVITY_AFM_HEALTH_URL ?? "http://127.0.0.1:11437/health";
...
async function main() {
  ...
  try {
    ...
    const direct = await callAfmPrepareTask(AFM_URL, {
      task: "Live low-memory v0 adapter check for Gemini Anti Gravity task prep.",
      intent: "verify",
      budgetTokens: 1000,
    ...
    const packet = await prepareTask({
      task: "live low-memory battery for gemini_antigravity_prepare_task using the v0 AFM adapter",
      useAfm: true,
      budgetTokens: 2000,
      vaultPath: root,
      limit: 2,
    ...
    const cli = spawnSync(
      ...
      ["dist/cli.js", "prepare", "--afm", "live CLI prepare task using v0 adapter"],
      {
        ...
        env: {
          ...process.env,
          GEMINI_ANTIGRAVITY_VAULT_PATH: root,
        },
        timeout: 60000,
      },
    );
    ...
    const outcome = await prepareOutcome({
      ...
      changedFiles: ["plugins/gemini-antigravity/src/task.ts"],
    ...
    const client = new Client({ name: "gemini-antigravity-live-prepare-test", version: "0.1.0" });
    try {
      await client.connect(transport);
      const tools = await client.listTools();
      assert.ok(tools.tools.some((tool) => tool.name === "gemini_antigravity_prepare_task"));
      assert.ok(tools.tools.some((tool) => tool.name === "gemini_antigravity_prepare_outcome"));
      const result = await client.callTool({
        name: "gemini_antigravity_prepare_task",
        arguments: {
