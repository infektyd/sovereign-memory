...
test("routeMemoryIntent allows automatic recall but not learning", () => {
  const recall = routeMemoryIntent("continue testing the Gemini Anti Gravity Memory plugin from prior context");
  assert.equal(recall.action, "recall");
  assert.equal(recall.automaticAllowed, true);
  assert.equal(recall.suggestedTool, "gemini_antigravity_recall");

  const learn = routeMemoryIntent("remember this decision in Gemini Anti Gravity Memory");
  assert.equal(learn.action, "learn");
  assert.equal(learn.automaticAllowed, false);
  assert.equal(learn.suggestedTool, "gemini_antigravity_learn");
});

test("assessLearningQuality rewards sourced durable notes and warns on weak notes", () => {
  const good = assessLearningQuality({
    title: "Gemini Anti Gravity recall ranking decision",
    content: "Gemini Anti Gravity recall should show vault context packs before broad daemon semantic results.",
    category: "decision",
    source: "unit-test",
  });
