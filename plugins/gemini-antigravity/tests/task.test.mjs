...
test("prepareTask builds a compact deterministic Gemini Anti Gravity task packet", async () => {
  const audits = [];
  ...
  assert.match(packet.contextMarkdown, /Gemini Anti Gravity Task Packet/);
  assert.equal(packet.afm.used, false);
  assert.equal(audits[0].tool, "gemini_antigravity_prepare_task");
});
...
test("prepareTask ranks fresh handoff notes above older sessions and explains why", async () => {
  const packet = await prepareTask(
    ...
    },
    {
      searchVault: async () => [
        ...
        vaultSource({
          relativePath: "wiki/sessions/20260425-gemini_antigravity-gemini-antigravity-plugin-backend-handoff-clean.md",
          wikilink: "[[wiki/sessions/20260425-gemini_antigravity-gemini-antigravity-plugin-backend-handoff-clean]]",
          title: "Gemini Anti Gravity Gemini Anti Gravity Memory plugin backend handoff clean",
  ...
  assert.match(packet.relevantSources[0].relativePath, /20260425-gemini_antigravity-gemini-antigravity-plugin-backend-handoff-clean/);
  ...
  const packet = await prepareOutcome(
    {
      ...
      changedFiles: ["plugins/gemini-antigravity/src/task.ts", "plugins/gemini-antigravity/tests/task.test.mjs"],
...
test("AFM outcome payload uses compact outcome instructions and redacts local paths", () => {
  const payload = buildAfmChatPayload({
    ...
    changedFiles: ["/Users/example/private/repo/file.ts", "plugins/gemini-antigravity/src/task.ts"],
  ...
  assert.match(body, /Return compact JSON only for Gemini Anti Gravity outcome prep/);
  assert.match(body, /plugins\/gemini-antigravity\/src\/task.ts/);
