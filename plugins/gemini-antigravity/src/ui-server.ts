...
import { buildStatusReport } from "./gemini_antigravity.js";
...
const DEFAULT_UI_HOST = process.env.GEMINI_ANTIGRAVITY_UI_HOST ?? "127.0.0.1";
const DEFAULT_UI_PORT = Number(process.env.GEMINI_ANTIGRAVITY_UI_PORT ?? "8765");
...
export function createUiServer(options: UiServerOptions = {}): UiServerHandle {
  ...
  if (!localBindHostAllowed(host)) {
    throw new Error(`Gemini Anti Gravity Memory console requires a local bind host, got: ${host}`);
  }
  ...
  const server = createServer(async (req, res) => {
    try {
      if (!localHostAllowed(req.headers.host)) {
        sendText(res, 403, "Gemini Anti Gravity Memory console only accepts local host requests.");
        return;
      }
      if (!localOriginAllowed(req.headers.origin) || !fetchSiteAllowed(req.headers["sec-fetch-site"])) {
        sendText(res, 403, "Gemini Anti Gravity Memory console only accepts same-origin local requests.");
        return;
      }
      ...
      if (req.method === "GET" && url.pathname === "/api/health") {
        sendJson(res, 200, {
          ...
          host,
          port,
          tools: ["gemini_antigravity_prepare_task", "gemini_antigravity_prepare_outcome", "gemini_antigravity_status", "gemini_antigravity_audit_tail"],
...
async function main(): Promise<void> {
  ...
  await app.start();
  console.log(`Gemini Anti Gravity Memory console: http://${app.host}:${app.port}/`);
}
