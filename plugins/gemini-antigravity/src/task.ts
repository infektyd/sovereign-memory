...
import { recallMemory } from "./gemini_antigravity.js";
import type { JsonResult, RecallResponse } from "./gemini_antigravity.js";
...
function deterministicPacket(input: {
  ...
  const currentState = [
    relevantSources.length > 0
      ...
      : "No matching Gemini Anti Gravity vault notes were found for this task.",
  ...
  const risks = [
    "Older broad semantic recall can outrank fresher Gemini Anti Gravity vault notes unless source ranking is explicit.",
  ...
  const contextMarkdown = [
    "# Gemini Anti Gravity Task Packet",
    `Task: ${input.task}`,
    `Intent: ${intent}`,
...
export function buildAfmChatPayload(payload: Record<string, unknown>): Record<string, unknown> {
  ...
  const content =
    purpose === "outcome"
      ? [
          "Return compact JSON only for Gemini Anti Gravity outcome prep.",
      ...
      : [
          "Return compact JSON only for Gemini Anti Gravity task prep.",
...
export async function prepareTask(input: PrepareTaskInput, deps: PrepareTaskDeps = {}): Promise<PreparedTaskPacket> {
  ...
  await audit(vaultPath, {
    tool: "gemini_antigravity_prepare_task",
...
function outcomeContextMarkdown(packet: PreparedOutcomePacket): string {
  return [
    "# Gemini Anti Gravity Outcome Packet",
    `Task: ${packet.task}`,
    `Profile: ${packet.profile}`,
    "## Summary",
    packet.summary,
