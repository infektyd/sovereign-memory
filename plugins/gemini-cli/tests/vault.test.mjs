...
test("ensureVault creates the Gemini CLI LLM wiki structure and schema", async () => {
  ...
  try {
    ...
    assert.match(schema, /Gemini CLI Gemini CLI Memory Vault/);
...
test("vaultFirstLearn writes a note, updates index, and appends audit logs", async () => {
  ...
  try {
    const result = await vaultFirstLearn({
      ...
      content: "Gemini CLI Memory daemon health is checked through /tmp/gemini_cli.sock.",
      category: "fact",
      source: "unit-test",
      agentId: "gemini_cli",
    ...
    assert.match(note, /agent: gemini_cli/);
    assert.match(note, /category: fact/);
    assert.match(note, /Gemini CLI Memory daemon health/);
    ...
    assert.match(log, /gemini_cli_learn/);
...
test("auditTail returns recent audit entries from daily logs", async () => {
  ...
  try {
    ...
    assert.match(tail.text, /gemini_cli_status/);
...
test("auditReport summarizes recent tool activity", async () => {
  ...
  try {
    ...
    assert.equal(report.tools.gemini_cli_recall, 1);
    assert.equal(report.tools.gemini_cli_learning_quality, 1);
    assert.deepEqual(report.recentSummaries, [
      "gemini_cli_recall: recall checked",
      "gemini_cli_learning_quality: quality checked",
    ]);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
...
test("searchVaultNotes ranks Gemini CLI wiki learnings for recall context", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "sm-search-"));
  try {
    await vaultFirstLearn({
      vaultPath: root,
      title: "Gemini CLI plugin full suite marker",
    ...
    assert.match(results[0].relativePath, /wiki\/sessions\/\d{8}-gemini-cli-plugin-full-suite-marker\.md$/);
    assert.match(results[0].wikilink, /\[\[wiki\/sessions\/\d{8}-gemini-cli-plugin-full-suite-marker\]\]/);
