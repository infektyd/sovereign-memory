...
test("prepareTask builds a compact deterministic Gemini CLI task packet", async () => {
  const audits = [];
  ...
  assert.match(packet.contextMarkdown, /Gemini CLI Task Packet/);
  assert.equal(packet.afm.used, false);
  assert.equal(audits[0].tool, "gemini_cli_prepare_task");
});
...
test("prepareTask ranks fresh handoff notes above older sessions and explains why", async () => {
    ...
    },
    {
      searchVault: async () => [
        ...
        vaultSource({
          ...
          title: "Gemini CLI Gemini CLI Memory plugin backend handoff clean",
  ...
  assert.match(packet.relevantSources[0].relativePath, /20260425-gemini_cli-gemini-cli-plugin-backend-handoff-clean/);
  ...
  const packet = await prepareOutcome(
    {
      ...
      changedFiles: ["plugins/gemini-cli/src/task.ts", "plugins/gemini-cli/tests/task.test.mjs"],
...
test("AFM outcome payload uses compact outcome instructions and redacts local paths", () => {
  const payload = buildAfmChatPayload({
    ...
    changedFiles: ["/Users/example/private/repo/file.ts", "plugins/gemini-cli/src/task.ts"],
  ...
  assert.match(body, /Return compact JSON only for Gemini CLI outcome prep/);
  assert.match(body, /plugins\/gemini-cli\/src\/task.ts/);
