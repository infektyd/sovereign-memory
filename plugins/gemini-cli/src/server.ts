...
import { formatRecall, learnMemory, recallMemory, statusAndAudit } from "./gemini_cli.js";
...
server.registerTool(
  "gemini_cli_prepare_task",
  {
    title: "Gemini CLI Prepare Task",
    description:
      "Build a compact Gemini CLI task packet from vault notes, daemon recall, constraints, and optional AFM distillation.",
    inputSchema: {
      task: z.string().min(1),
...
server.registerTool(
  "gemini_cli_prepare_outcome",
  {
    title: "Gemini CLI Prepare Outcome",
    description:
...
server.registerTool(
  "gemini_cli_status",
  {
    title: "Gemini CLI Memory Status",
    description: "Check Gemini CLI daemon, AFM health, Gemini CLI vault, and audit state.",
    inputSchema: {
      vaultPath: z.string().optional(),
    },
  },
...
server.registerTool(
  "gemini_cli_route",
  {
    title: "Gemini CLI Memory Intent Router",
...
server.registerTool(
  "gemini_cli_recall",
  {
    title: "Gemini CLI Memory Recall",
    description: "Recall Gemini CLI Memory context and log the lookup in the Gemini CLI vault.",
    inputSchema: {
...
server.registerTool(
  "gemini_cli_learn",
  {
    title: "Gemini CLI Memory Learn",
    description: "Write a Gemini CLI vault note first, then store the learning through Gemini CLI Memory.",
    inputSchema: {
...
server.registerTool(
  "gemini_cli_learning_quality",
  {
    title: "Gemini CLI Learning Quality",
    description: "Review a potential memory before writing it to the Gemini CLI vault or Gemini CLI daemon.",
    inputSchema: {
      title: z.string().min(1),
      content: z.string().min(1),
      category: z.string().optional(),
  ...
  async ({ title, content, category, source, vaultPath }) => {
    const quality = assessLearningQuality({ title, content, category, source });
    await recordAudit(vaultPath ?? DEFAULT_VAULT_PATH, {
      tool: "gemini_cli_learning_quality",
      summary: title,
      details: { quality },
    });
...
server.registerTool(
  "gemini_cli_vault_write",
  {
    title: "Gemini CLI Vault Write",
    description: "Write a structured Gemini CLI Obsidian vault page without storing it as a durable learning.",
    inputSchema: {
      title: z.string().min(1),
      content: z.string().min(1),
...
server.registerTool(
  "gemini_cli_audit_report",
  {
    title: "Gemini CLI Audit Report",
    description: "Summarize recent Gemini CLI Memory tool activity for transparent self-auditing.",
    inputSchema: {
...
server.registerTool(
  "gemini_cli_audit_tail",
  {
    title: "Gemini CLI Audit Tail",
    description: "Show recent Gemini CLI Memory audit entries from the Gemini CLI vault.",
    inputSchema: {
