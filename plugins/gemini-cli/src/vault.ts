...
function scoreVaultNote(query: string, terms: string[], relativePath: string, title: string, markdown: string): number {
  ...
  if (/gemini_cli_learning:\s*true/i.test(markdown)) score += 2;
  return score;
}

function schemaContent(): string {
  return `# Gemini CLI Gemini CLI Memory Vault

This vault is Gemini CLI's local-first LLM wiki for Gemini CLI Memory.
...
- Treat \`wiki/\` as Gemini CLI-maintained synthesis. Keep pages short, sourced, and linked with Obsidian wikilinks.
...
- Update \`index.md\` and append to \`log.md\` whenever Gemini CLI creates or learns from a note.
...
function indexContent(): string {
  return `# Gemini CLI Gemini CLI Memory Index

This index is maintained by the Gemini CLI Memory Gemini CLI plugin.
...
function logContent(): string {
  return `# Gemini CLI Memory Gemini CLI Log

Append-only audit of Gemini CLI memory operations.
...
export async function recordAudit(vaultPath: string, entry: AuditEntry): Promise<string> {
  ...
  if (!(await exists(dailyPath))) {
    await writeFile(dailyPath, `# ${date} Gemini CLI Memory Audit\n\n`, "utf8");
  }
...
export async function vaultFirstLearn(input: LearnInput): Promise<VaultWriteResult> {
  const result = await writeVaultPage({
    ...
    source: input.source,
    frontmatter: {
      agent: input.agentId ?? "gemini_cli",
      category: input.category ?? "general",
      gemini_cli_learning: true,
    },
  });
